 ...
 * public     로그인 없이 열려 있다 — 로그인·가입 화면·인증 라우트·헬스체크·정적 자산
 ...
 *
 * **`/pending`은 여기에 없다.** 승인을 기다리는 사람은 **로그인한 상태**라 이 표의 축
 * (「로그인 없이 지나갈 수 있는가」)에 속하지 않는다. 공개로 두면 로그아웃 상태에서도
 * 열리고, `proxy`가 세션 갱신을 건너뛰어 그 계정의 토큰이 조용히 만료된다. 대기 여부로
 * 가르는 것은 `pending-gate.ts`이고, 그것은 `profiles`를 읽어야 알 수 있어 여기서 잴 수 없다.
 *
...
const PUBLIC_PREFIXES = [
  '/login',
  // 가입 화면이 로그인을 요구하면 아무도 계정을 만들 수 없다 (T11). 가입 라우트는
  // `/api/auth/signup`이라 아래 줄에 이미 걸린다 — 접두사를 더 넓히지 않는다
  '/signup',
  '/api/auth',
