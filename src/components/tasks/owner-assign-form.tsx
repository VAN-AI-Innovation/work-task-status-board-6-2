 ...
 * ## 폼을 둘로 나눈 이유
 *
 * 상태·진행률과 한 폼에 묶지 않았다. **누가 쓰는지가 다르다** — 진행률은 그 업무를 들고 있는
 * 사람이 적고(`canEditProgress`), 담당자는 일을 맡기는 사람이 정한다(`canAssignOwner`).
 * 한 폼이면 대표·실장에게 진행률 칸이 딸려 오고, 부원에게는 담당자 칸이 딸려 온다.
 *
 * ## 후보를 화면이 고르지 않는다
 *
 ...
 *
 * ⚠ 그래서 화면이 그 사실을 한 줄로 적는다: **공동 담당자는 자기 화면에서 이 업무를 보지
 * 못한다.** 적지 않으면 팀장은 세 명을 지정해 놓고 두 명이 못 보는 것을 사고로 읽는다.
 *
  ...
  return (
    <div className="border-line rounded border p-3">
      ...
      <p className="text-ink-muted mt-3 text-xs">
        공동 담당자는 이름으로만 남습니다 — 그 사람의 화면에는 이 업무가 뜨지 않습니다. 다음
