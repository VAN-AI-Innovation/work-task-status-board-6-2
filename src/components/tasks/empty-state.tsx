 ...
 * 생긴다. 세 번째 갈래(`unlinked-member`)는 로그인한 부원의 계정이 시트 담당자와 이어지지
 * 않은 경우이고(`unknown_owner` · `PLAN.md` 결정 D), 그 사람에게 「아직 데이터가 없습니다」를
 * 띄우면 전사 데이터가 멀쩡히 있는데도 시트를 올리러 간다.
 *
...
const MESSAGES: Readonly<Record<EmptyReason, string>> = {
  ...
  'unlinked-member':
    '담당자로 연결된 계정이 없어 표시할 업무가 없습니다. 시트의 담당자 이름과 계정 연결이 필요합니다.',
};
