  ...
  return (
    <section className="border-line bg-panel @container flex flex-col rounded-md border p-4">
      ...
      {/*
        ...
        *
        * **높이가 여기서 고정된다.** 접힌 묶음 다섯 줄이 그 높이다 (`2.25rem` 줄 다섯 + 줄
        * 사이 `0.5rem` 넷). 데이터가 늘거나 묶음을 펼치면 목록은 끝없이 길어질 수 있고,
        * 그러면 같은 행의 옆 카드가 그만큼 빈 채로 늘어난다 — 묶음을 펼칠 때마다 상태 분포
        * 아래가 희게 벌어지던 것이 그것이다. 그래서 `max-h`가 아니라 `h`다: 펼쳐도 카드
        * 높이가 그대로고 **목록만** 스크롤한다. 반대쪽(옆 카드를 따라 늘어나는 것)은
        * `FIXED_HEIGHT`가 막는다.
        *
        ...
        */}
      <div className="mt-3 -mx-2 h-[calc(5*2.25rem+4*0.5rem)] overflow-y-auto px-2 @2xl:columns-2 @2xl:gap-x-6">
        {groups.map((group) => {
