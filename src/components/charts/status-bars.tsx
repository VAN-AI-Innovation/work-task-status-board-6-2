 ...
 * 통째로 빈 채 끌려 나온다.
 *
 * 그래서 캔버스가 **자기 칸의 높이를 받기만 한다**: 카드가 `flex h-full flex-col`이고 이
 * 칸이 `flex-1`이라, 높이는 카드가 정하고 캔버스는 그 안을 채운다 (`h-full`). 되먹임이
 * 없는 것은 카드 높이가 행에서 이미 확정되기 때문이고, 행이 짧을 때의 바닥은 `min-h`가
 * 잡는다 — 막대 여섯 줄과 x축이 들어갈 만큼이다. 늘어난 세로가 막대를 굵게 만들지 않도록
 * `maxBarThickness`가 아래에서 막는다.
 *
...
export function StatusBars({ series }: { series: ChartSeries }) {
  return (
    // 카드가 준 세로를 채우되, 바닥은 막대 여섯 줄 + x축 눈금이다 (머리말)
    <div className="h-full min-h-[152px]">
      <Bar
        data={{
